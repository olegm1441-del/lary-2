...
class SupportLetterServiceTest(unittest.TestCase):
    ...
    def test_ai_prompt_and_output_do_not_put_cofinance_amount_into_support_block(self):
        ai_value = {
            "ai_value_block": "Видим необходимость проекта в следующем:\n1. Проект помогает молодежи включиться в культурную практику.\n2. Проект усиливает интерес к локальной истории.\n3. Проект создает понятный маршрут участия.\nВидим особенным этот проект для нашей территории.",
        }
        ai_support = {
            "ai_support_block": "Информационная поддержка: готовы разместить анонсы проекта на своих площадках. Оценка вклада: 300 тыс. рублей. Финансовый вклад партнера составляет 300 000 рублей.",
        }
        ...
        prompts = "\n\n".join(call.args[0] for call in ai.call_args_list)
        self.assertIn("Не пиши сумму софинансирования", prompts)
        self.assertNotIn("Оценка вклада в рублях: 600 000", prompts)
        self.assertNotIn("600 000", prompts)

        text = docx_text(result.docx_bytes)
        self.assertIn("Информационная поддержка: готовы разместить анонсы проекта на своих площадках.", text)
        self.assertIn("Оцениваем наш вклад в 600 000 рублей", text)
        self.assertNotIn("Оценка вклада: 300 тыс. рублей", text)
        self.assertNotIn("Финансовый вклад партнера составляет 300 000 рублей", text)
        self.assertNotIn("300 тыс", text)
        self.assertNotIn("300 000 рублей", text)
