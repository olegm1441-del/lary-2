...
export default async function ModulePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ example?: string }> }) {
  ...
  const formDescription =
    laryModule.slug === "support-letter"
      ? "Пишите коротко и фактами. Название партнера, вклад и подписант вставляются в письмо без нейросети. GigaChat аккуратно сформулирует только два текстовых блока: значимость проекта и описание поддержки."
